...
public:
    ...
    virtual ~PFArray();
    PFArray<T>& operator=(const PFArray<T>& other);
...
public:
    ...
    //Big Three
    PFArrayB(const PFArrayB<T>& other);
    virtual ~PFArrayB();
    PFArrayB& operator=(const PFArrayB<T>& other);

    //Getters
    int getUsedB()const{return usedB;}
...
template<class T>
PFArrayB<T>::PFArrayB(const PFArrayB<T>& other)
{
    *this=other;
}
...
template<class T>
PFArrayB<T>& PFArrayB<T>::operator=(const PFArrayB<T>& other)
{
    if(pointerB!=nullptr && usedB!=other.usedB)
    {
        delete[] pointerB;
        pointerB=nullptr;
    }
    ...
    {
        if(pointerB==nullptr)
            pointerB=new T[other.getCap()];

        usedB=other.usedB;

        for(int i=0; i<getUsedB(); ++i)
            pointerB[i]=other.pointerB[i];

        PFArray<T>::operator=(other);
...
void PFArrayB<T>::backup()
{
    ...
    for(int i=0; i<getUsedB(); ++i)
        pointerB[i]=PFArray<T>::operator[](i);
}
...
void PFArrayB<T>::restore()
{
    ...
    if(getUsedB()>this->getCap())
        this->setCapacity(getUsedB());

    this->used=getUsedB();

    for(int i=0; i<getUsedB(); ++i)
        PFArray<T>::operator[](i)=pointerB[i];
}
